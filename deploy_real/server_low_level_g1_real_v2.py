...
# Simple stand balance for STAND_TEST / SAFE_STAND only.
# Keep this very small for first real test.
STAND_BALANCE_ENABLE = True
STAND_PITCH_REF = 0.0
STAND_PITCH_KP = 0.08
STAND_PITCH_KD = 0.015
STAND_PITCH_MAX = 0.04

# If the robot gets worse, change this to -1.0 and test again.
STAND_PITCH_SIGN = 1.0
...
class DeploymentLogger:
    ...
    def _to_npz_array(self, key, values):
        if not values:
        ...
        try:
            ...
            try:
                ...
                # Last-resort fallback: keep every sample instead of dropping
                # the whole NPZ when one field has a ragged shape.
    ...
    def flush_npz(self):
        if not self.npz_path:
            return
        tmp_path = f"{self.npz_path}.tmp"
        payload = {
            key: self._to_npz_array(key, value)
        ...
        os.replace(tmp_path, self.npz_path)
    ...
    def close(self):
        ...
        def _defer_interrupt(signum, _frame):
            ...
            print(f"[LOG] Deferring signal {signum} until logs are saved.")
        ...
        if interrupted["value"]:
            raise KeyboardInterrupt("interrupted after logs were saved")
...
class RealTimePolicyControllerV2:
    ...
    def _apply_stand_pitch_balance(self, base_target, rpy=None, ang_vel=None):
        """Small IMU pitch compensation for standing only.

        This does not affect TRACKING policy action directly.
        It only modifies the default standing target in STAND_TEST / SAFE_STAND.
        """
        target = np.asarray(base_target, dtype=np.float32).copy()
        if not STAND_BALANCE_ENABLE or rpy is None or ang_vel is None:
            return target

        pitch = float(rpy[1]) - STAND_PITCH_REF
        pitch_rate = float(ang_vel[1])

        comp = STAND_PITCH_SIGN * (
            STAND_PITCH_KP * pitch + STAND_PITCH_KD * pitch_rate
        )
        comp = float(np.clip(comp, -STAND_PITCH_MAX, STAND_PITCH_MAX))

        # Policy/default order:
        # 0 L_hip_pitch, 4 L_ankle_pitch, 6 R_hip_pitch, 10 R_ankle_pitch
        # Use ankle as the main balance joint, hip as a smaller support.
        target[4] += comp
        target[10] += comp
        target[0] += 0.5 * comp
        target[6] += 0.5 * comp
    ...
    def _desired_for_mode(self, policy_target, rpy=None, ang_vel=None):
        now = time.monotonic()
        ...
        if self.mode == "RECOVERY":
            ...
            return desired, 1.0
        self.ramp = 1.0
        return self._apply_stand_pitch_balance(self.default_dof_pos, rpy, ang_vel), 1.0
    ...
    def _run_stand_test(self):
        ...
        for _ in range(steps):
            ...
            stand_target = self._apply_stand_pitch_balance(
                self.default_dof_pos, rpy, ang_vel
            )
            result = self.safety_filter.apply(stand_target, 1.0)
    ...
    def run(self):
        ...
        try:
            ...
            while self.max_steps is None or self.frame_id < self.max_steps:
                ...
                desired, action_ramp = self._desired_for_mode(policy_target, rpy, ang_vel)
    ...
    def _blend_to_default_pose(self):
        ...
        for _ in range(steps):
            ...
            target = self.safety_filter.apply(stand_target, 1.0).target
