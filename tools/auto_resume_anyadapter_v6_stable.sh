...
# Stable Any2Track V6 training: in-place rewards + bounded layer adapters.
set -uo pipefail
...
PROJ="g1_twist_any2track_v6_stable_0529"
EXPTID="twist_any2track_v6_stable_0529_train"
DEVICE="cuda:0"
NUM_ENVS=4096
...
LOG_FILE="$PROJECT_DIR/tools/auto_resume_anyadapter_v6_stable.log"
LOCK_FILE="$PROJECT_DIR/tools/auto_resume_anyadapter_v6_stable.lock"

exec 9>"$LOCK_FILE"
if ! flock -n 9; then
    echo "Another stable V6 auto-resume launcher is already running." >&2
    exit 1
fi
...
log "TWIST Any2Track stable V6 auto-resume started"
...
while [ "$restart_count" -lt "$MAX_RESTARTS" ]; do
    ...
    if [ "$current" -gt 0 ]; then
        ...
        log "Starting fresh stable V6 training; iterations=$remaining"
    fi
