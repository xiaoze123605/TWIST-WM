...
# Fixed 0529-based Any2Track-style V6 training launcher. Simulation only.
set -uo pipefail
...
# V6 keeps two large 7001-D rollout tensors on GPU.  Expandable CUDA segments
# reduce allocation failures caused by fragmentation across automatic restarts.
export PYTORCH_CUDA_ALLOC_CONF="${PYTORCH_CUDA_ALLOC_CONF:-expandable_segments:True}"

TASK="g1_stu_anyadapter_v6"
PROJ="g1_twist_any2track_v6_0529"
EXPTID="twist_any2track_v6_0529_train"
DEVICE="cuda:0"
NUM_ENVS=4096
...
LOG_FILE="$PROJECT_DIR/tools/auto_resume_anyadapter_v6.log"
LOCK_FILE="$PROJECT_DIR/tools/auto_resume_anyadapter_v6.lock"

exec 9>"$LOCK_FILE"
if ! flock -n 9; then
    echo "Another V6 auto-resume launcher is already running." >&2
    exit 1
fi
...
log "TWIST Any2Track V6 auto-resume started"
...
while [ "$restart_count" -lt "$MAX_RESTARTS" ]; do
    ...
    if [ "$current" -gt 0 ]; then
        ...
        log "Starting fresh V6 training; iterations=$remaining"
    fi
    ...
    log "Training exited code=$exit_code iteration=$current; restart in ${RESTART_DELAY}s ($restart_count/$MAX_RESTARTS)"
    sleep "$RESTART_DELAY"
done
