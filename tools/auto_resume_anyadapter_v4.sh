...
# Fixed TWIST-AnyAdapter V4 training launcher.
...
TASK="g1_stu_anyadapter_v4"
PROJ="g1_twist_anyadapter_v4_0529"
EXPTID="twist_anyadapter_v4_0529_train"
DEVICE="cuda:0"
NUM_ENVS=4096
...
LOG_FILE="$PROJECT_DIR/tools/auto_resume_anyadapter_v4.log"
LOCK_FILE="$PROJECT_DIR/tools/auto_resume_anyadapter_v4.lock"

exec 9>"$LOCK_FILE"
if ! flock -n 9; then
    echo "Another V4 auto-resume launcher is already running." >&2
    exit 1
fi
...
log "TWIST-AnyAdapter V4 auto-resume started"
...
while [ "$restart_count" -lt "$MAX_RESTARTS" ]; do
    ...
    if [ "$current" -gt 0 ]; then
        ...
        log "Starting a fresh 0529-based V4 run; iterations=$remaining"
    fi
